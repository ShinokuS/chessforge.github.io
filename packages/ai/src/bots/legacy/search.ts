...
export type ChooseOptions = {
  ...
  ttBits?: number;
  /**
   * Arena / self-play throughput: score root moves with static eval only
   * (no alphabeta / quiesce). Skill noise still applies. Extremely fast.
   */
  batch?: boolean;
};
...
/**
 * Fast root-only chooser for batch self-play (static eval + skill noise).
 * At skill 0 picks a deterministic pseudo-random legal move.
 * No alphabeta / quiesce — suitable for thousands of arena games.
 */
function chooseCommandBatch(state: MatchState, skill: number): GameCommand {
  ...
  for (const m of rootMoves) {
    const result = applyCommand(state, moveToCommand(m));
    if (!result.ok) continue;
    const adjusted = evaluate(result.state, stm) + rootNoise(m, skill, 1);
...
export function chooseCommand(state: MatchState, options: ChooseOptions = {}): GameCommand {
  ...
  if (options.batch) {
    return chooseCommandBatch(state, skill);
  }
  const ctx = createContext(timeMs, nodeLimit, ttBits);
